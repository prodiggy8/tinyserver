...
def build_router(store, registry):
    """Build a Router wired to the given chat store/registry. Factored out
    of module scope so tests can construct isolated instances (an
    ephemeral `MessageStore`, a fresh `ConnectionRegistry`) instead of
    sharing the process-wide singletons below — e.g. to test persistence
    across a simulated server restart without touching the real
    `data/messages.jsonl`.
    """
    ...
    def ws_handler(request):
        ...
        connection's frame loop on this (the hijacking) thread until it
        closes. Returns the HIJACKED sentinel so the HTTP loop never
        touches the socket again; on a failed handshake or a full registry
        it returns a normal (status, headers, body) tuple instead, leaving
        HTTP keep-alive intact.

        Ordering is load-bearing: `registry.register` starts this
        connection's writer thread, which becomes the only thread allowed
        to write to the socket — so the raw 101 response MUST be sent, in
        full, before `register` is called, or the writer thread could race
        it onto the wire. The capacity check is therefore a separate
        up-front peek (`at_capacity`, racy by design — see its docstring)
        rather than relying solely on `register`'s atomic check, since that
        check is too late to prevent a 503 body from being sent after a
        101 already went out.
        """
        try:
        ...
        if conn is None:
            # Capacity filled in the tiny race window between the check
            # above and this call; the 101 line is already on the wire, so
            # the best we can do is close without a body.
            try:
                request.sock.close()
            except OSError:
                pass
